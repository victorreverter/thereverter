...
function IntroPost(PostData) {
  ...
  // onClick={()=>navigate('../posts/'+IntroData.url_name)}
  // onClick={() => history.push(`/posts/${IntroData.url_name}`)}

  const postRoute = '../posts/' + IntroData.url_name;

  return (
    <Link to={postRoute}>
      <div id="intro_post_container" className='grid grid-cols-1
      md:grid-cols-2 px-2.5 md:px-6
      lg:px-0 gap-8 cursor-pointer' >
